 ...
 */
@AutoRegister
internal object MythicMobsIVHook: BaseRealizer ("mythicMobsIVHook"), Awakeable{
    ...
    override fun onEnable() {
        ...
        {
            console().sendMessage(""" &6前置 &9[ &MythicMobsIV &9] &6插件已加载... """)
            isLoad = true
        }
    }
    ...
    fun getMythicItem (name: String): ItemStack
    {
        defaultItem.takeIf { it.itemMeta != null }?.apply {
            ...
            if (isLoad) meta!!.setDisplayName(" &4 提供的 MythicMobsIV 物品 id &9[&f$name&9] &4不存在...")
            else meta!!.setDisplayName(" &4前置 &9[ &MythicMobsIV &9] &4插件未加载 &9, &4请检查...")
        ...
        if (!isLoad)
        {
            console().sendMessage(""" &4前置 &9[ &MythicMobsIV &9] &4插件未加载 &9, &4请检查... """)
            return defaultItem.clone ()
        }

        return MythicMobs.inst().itemManager.getItemStack(name) ?: defaultItem.clone ()
    }
}